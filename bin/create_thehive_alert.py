...
def create_alert(config, row):
	...
	sourceRefField = config.get('sourceRefField') # it's a little weird but this grabs the field name
	sourceRef = row.pop(sourceRefField) # grabs that field's value and assigns it to our sourceRef 
	
	# grab our date field
	dateField = config.get('dateField')
	date = row.pop(dateField)
	
	# time to exclude those particular fields
	ignore = config.get('ignoredFields', [])
	row = {key: value for key, value in row.iteritems() if not key in ignore}
